...
public class AppUpdateManager {
    ...
    public static void showAppUpdateDialogIfNeed(FragmentActivity activity, boolean silent) {
        String versionNameStr = Utility.getVersionName(activity);
        float versionName = Float.parseFloat(versionNameStr);
        float forceVersion = getForceUpdateVersion(activity);
        float normalVersion = getNormalUpdateVersion(activity);
        String url = getApkDownloadUrl(activity);

        StringBuilder logSb = new StringBuilder();
        logSb.append("-------doUpdateWork------" + " | ");
        logSb.append("versionName : " + versionName + " | ");
        logSb.append("forceVersion : " + forceVersion + " | ");
        logSb.append("normalVersion : " + normalVersion + " | ");

        if (versionName < forceVersion) {
            // 显示强制更新对话框
...
//            showAppUpdateDialog(false, activity, url, normalVersion);
    ...
    private static void showAppUpdateDialog(boolean force, FragmentActivity activity, String downloadUrl, float expectVersionCode) {
        if (activity == null) {
    ...
    public static void downloadApk(AppUpdateDialogFragment dialogFragment, String downloadUrl, float expectVersionCode) {
    ...
    public static boolean isDownloadedCorrectApk(Context context, float remoteVersion) {
        ...
        if (!TextUtils.isEmpty(path)) {
            ...
            if (apkFile.exists()) {
                ...
                if (packageInfo != null && packageInfo.packageName.equals(context.getPackageName())) {
                    downloadFinishedAndCorrect = Float.parseFloat(packageInfo.versionName) >= remoteVersion;
                    logSb.append("downloaded apk file versionName : " + packageInfo.versionName + ", remoteVersion:" + remoteVersion).append(" | ");
                }
            }
        } else {
    ...
    /** 保存应用更新信息 */
    public static void saveAppUpdateInfo(Context context, String url, float normalVersion, float forceVersion) {
